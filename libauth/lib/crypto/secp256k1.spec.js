/* eslint-disable max-lines, @typescript-eslint/max-params, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-argument */
// cspell:ignore recid
/* global Buffer */
import { randomBytes } from 'crypto';
import test from 'ava';
import { getEmbeddedSecp256k1Binary, instantiateSecp256k1, instantiateSecp256k1Bytes, secp256k1 as internalSecp256k1, Secp256k1Error, } from '../lib.js';
...
const setupElliptic = (privateKey) => {
    ...
    const pubUncompressed = new Uint8Array(key.getPublic().encode('array', false));
...
const fcUint8Array = (minLength, maxLength) => fc
    .array(fc.integer({ max: 255, min: 0 }), { maxLength, minLength })
    .map((a) => Uint8Array.from(a));
const fcUint8Array32 = () => fcUint8Array(32, 32);
const fcValidPrivateKey = fcUint8Array32().filter((generated) => internalSecp256k1.validatePrivateKey(generated));
...
test('[crypto] secp256k1.addTweakPrivateKey', async (t) => {
    ...
    t.deepEqual(secp256k1.addTweakPrivateKey(privkey, Buffer.alloc(32, 255)), Secp256k1Error.addTweakPrivateKey);
});
test('[fast-check] [crypto] secp256k1.addTweakPrivateKey', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        t.deepEqual(secp256k1.addTweakPrivateKey(privateKey, keyTweakVal), new Uint8Array(secp256k1Node.privateKeyTweakAdd(privateKey, keyTweakVal)));
    });
    t.notThrows(() => {
...
test('[crypto] secp256k1.addTweakPublicKeyCompressed', (t) => {
    t.deepEqual(internalSecp256k1.addTweakPublicKeyCompressed(pubkeyCompressed, keyTweakVal), pubkeyTweakedAddCompressed);
    t.deepEqual(internalSecp256k1.addTweakPublicKeyCompressed(new Uint8Array(65), keyTweakVal), Secp256k1Error.unparsablePublicKey);
    t.deepEqual(internalSecp256k1.addTweakPublicKeyCompressed(pubkeyCompressed, Buffer.alloc(32, 255)), Secp256k1Error.addTweakPublicKey);
});
test('[fast-check] [crypto] secp256k1.addTweakPublicKeyCompressed', async (t) => {
    ...
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        ...
        t.deepEqual(secp256k1.addTweakPublicKeyCompressed(pubkeyC, keyTweakVal), new Uint8Array(secp256k1Node.publicKeyTweakAdd(pubkeyC, keyTweakVal, true)));
    });
    t.notThrows(() => {
        fc.assert(equivalentToSecp256k1Node);
    });
...
test('[crypto] secp256k1.addTweakPublicKeyUncompressed', (t) => {
    t.deepEqual(internalSecp256k1.addTweakPublicKeyUncompressed(pubkeyUncompressed, keyTweakVal), pubkeyTweakedAddUncompressed);
    t.deepEqual(internalSecp256k1.addTweakPublicKeyUncompressed(new Uint8Array(65), keyTweakVal), Secp256k1Error.unparsablePublicKey);
    t.deepEqual(internalSecp256k1.addTweakPublicKeyUncompressed(pubkeyCompressed, Buffer.alloc(32, 255)), Secp256k1Error.addTweakPublicKey);
});
test('[fast-check] [crypto] secp256k1.addTweakPublicKeyUncompressed', async (t) => {
    ...
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        ...
        t.deepEqual(secp256k1.addTweakPublicKeyUncompressed(pubkeyU, keyTweakVal), new Uint8Array(secp256k1Node.publicKeyTweakAdd(pubkeyU, keyTweakVal, false)));
    });
    t.notThrows(() => {
        fc.assert(equivalentToSecp256k1Node);
    });
...
test('[crypto] secp256k1.compressPublicKey', (t) => {
    t.deepEqual(internalSecp256k1.compressPublicKey(pubkeyUncompressed), pubkeyCompressed);
    t.deepEqual(internalSecp256k1.compressPublicKey(new Uint8Array(65)), Secp256k1Error.unparsablePublicKey);
});
test('[fast-check] [crypto] secp256k1.compressPublicKey', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const reversesUncompress = fc.property(fcValidPrivateKey, (privateKey) => {
        const pubkeyC = secp256k1.derivePublicKeyCompressed(privateKey);
        t.deepEqual(pubkeyC, secp256k1.compressPublicKey(secp256k1.uncompressPublicKey(pubkeyC)));
    });
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        const pubkeyU = secp256k1.derivePublicKeyUncompressed(privateKey);
        t.deepEqual(secp256k1.compressPublicKey(pubkeyU), new Uint8Array(secp256k1Node.publicKeyConvert(pubkeyU, true)));
    });
    const equivalentToElliptic = fc.property(fcValidPrivateKey, (privateKey) => {
        const pubkeyU = secp256k1.derivePublicKeyUncompressed(privateKey);
        t.deepEqual(secp256k1.compressPublicKey(pubkeyU), new Uint8Array(ec.keyFromPublic(pubkeyU).getPublic().encodeCompressed()));
    });
    t.notThrows(() => {
...
test('[crypto] secp256k1.derivePublicKeyCompressed', (t) => {
    t.deepEqual(internalSecp256k1.derivePublicKeyCompressed(privkey), pubkeyCompressed);
    t.deepEqual(internalSecp256k1.derivePublicKeyCompressed(secp256k1OrderN), Secp256k1Error.derivePublicKeyFromInvalidPrivateKey);
});
test('[fast-check] [crypto] secp256k1.derivePublicKeyCompressed', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const isEquivalentToDeriveUncompressedThenCompress = fc.property(fcValidPrivateKey, (privateKey) => {
        const pubkeyU = secp256k1.derivePublicKeyUncompressed(privateKey);
        const pubkeyC = secp256k1.derivePublicKeyCompressed(privateKey);
        t.deepEqual(pubkeyC, secp256k1.compressPublicKey(pubkeyU));
    });
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        t.deepEqual(secp256k1.derivePublicKeyCompressed(privateKey), new Uint8Array(secp256k1Node.publicKeyCreate(privateKey, true)));
    });
    const equivalentToElliptic = fc.property(fcValidPrivateKey, (privateKey) => {
...
test('[crypto] secp256k1.derivePublicKeyUncompressed', (t) => {
    t.deepEqual(internalSecp256k1.derivePublicKeyUncompressed(privkey), pubkeyUncompressed);
    t.deepEqual(internalSecp256k1.derivePublicKeyUncompressed(secp256k1OrderN), Secp256k1Error.derivePublicKeyFromInvalidPrivateKey);
});
test('[fast-check] [crypto] secp256k1.derivePublicKeyUncompressed', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const isEquivalentToDeriveCompressedThenUncompress = fc.property(fcValidPrivateKey, (privateKey) => {
        const pubkeyC = secp256k1.derivePublicKeyCompressed(privateKey);
        const pubkeyU = secp256k1.derivePublicKeyUncompressed(privateKey);
        t.deepEqual(pubkeyU, secp256k1.uncompressPublicKey(pubkeyC));
    });
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        t.deepEqual(secp256k1.derivePublicKeyUncompressed(privateKey), new Uint8Array(secp256k1Node.publicKeyCreate(privateKey, false)));
    });
    const equivalentToElliptic = fc.property(fcValidPrivateKey, (privateKey) => {
...
test('[crypto] secp256k1.malleateSignatureDER', (t) => {
    t.deepEqual(internalSecp256k1.malleateSignatureDER(sigDER), sigDERHighS);
});
test('[fast-check] [crypto] secp256k1.malleateSignatureDER', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const malleationIsJustNegation = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, message) => {
...
test('[crypto] secp256k1.malleateSignatureCompact', (t) => {
    t.deepEqual(internalSecp256k1.malleateSignatureCompact(sigCompact), sigCompactHighS);
});
...
test('[crypto] secp256k1.mulTweakPrivateKey', (t) => {
    t.deepEqual(internalSecp256k1.mulTweakPrivateKey(privkey, keyTweakVal), privkeyTweakedMul);
    t.deepEqual(internalSecp256k1.mulTweakPrivateKey(privkey, Buffer.alloc(32, 255)), Secp256k1Error.mulTweakPrivateKey);
});
test('[fast-check] [crypto] secp256k1.mulTweakPrivateKey', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        t.deepEqual(secp256k1.mulTweakPrivateKey(privateKey, keyTweakVal), new Uint8Array(secp256k1Node.privateKeyTweakMul(privateKey, keyTweakVal)));
    });
    t.notThrows(() => {
        fc.assert(equivalentToSecp256k1Node);
    });
...
test('[crypto] secp256k1.mulTweakPublicKeyCompressed', (t) => {
    t.deepEqual(internalSecp256k1.mulTweakPublicKeyCompressed(pubkeyCompressed, keyTweakVal), pubkeyTweakedMulCompressed);
    t.deepEqual(internalSecp256k1.mulTweakPublicKeyCompressed(new Uint8Array(65), keyTweakVal), Secp256k1Error.unparsablePublicKey);
    t.deepEqual(internalSecp256k1.mulTweakPublicKeyCompressed(pubkeyCompressed, Buffer.alloc(32, 255)), Secp256k1Error.mulTweakPublicKey);
});
test('[fast-check] [crypto] secp256k1.mulTweakPublicKeyCompressed', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        const pubkeyC = secp256k1.derivePublicKeyCompressed(privateKey);
        t.deepEqual(secp256k1.mulTweakPublicKeyCompressed(pubkeyC, keyTweakVal), new Uint8Array(secp256k1Node.publicKeyTweakMul(pubkeyC, keyTweakVal, true)));
    });
    t.notThrows(() => {
        fc.assert(equivalentToSecp256k1Node);
    });
...
test('[crypto] secp256k1.mulTweakPublicKeyUncompressed', (t) => {
    t.deepEqual(internalSecp256k1.mulTweakPublicKeyUncompressed(pubkeyUncompressed, keyTweakVal), pubkeyTweakedMulUncompressed);
    t.deepEqual(internalSecp256k1.mulTweakPublicKeyUncompressed(new Uint8Array(65), keyTweakVal), Secp256k1Error.unparsablePublicKey);
    t.deepEqual(internalSecp256k1.mulTweakPublicKeyUncompressed(pubkeyCompressed, Buffer.alloc(32, 255)), Secp256k1Error.mulTweakPublicKey);
});
test('[fast-check] [crypto] secp256k1.mulTweakPublicKeyUncompressed', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        const pubkeyU = secp256k1.derivePublicKeyUncompressed(privateKey);
        t.deepEqual(secp256k1.mulTweakPublicKeyUncompressed(pubkeyU, keyTweakVal), new Uint8Array(secp256k1Node.publicKeyTweakMul(pubkeyU, keyTweakVal, false)));
    });
    t.notThrows(() => {
        fc.assert(equivalentToSecp256k1Node);
    });
...
test('[crypto] secp256k1.normalizeSignatureCompact', (t) => {
    t.deepEqual(internalSecp256k1.normalizeSignatureCompact(sigCompactHighS), sigCompact);
});
test('[fast-check] [crypto] secp256k1.normalizeSignatureCompact', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const malleateThenNormalizeEqualsInitial = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
    ...
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        const sig = secp256k1.signMessageHashCompact(privateKey, hash);
        const malleated = secp256k1.malleateSignatureCompact(sig);
        t.deepEqual(secp256k1.normalizeSignatureCompact(malleated), new Uint8Array(secp256k1Node.signatureNormalize(malleated)));
    });
    t.notThrows(() => {
...
test('[fast-check] [crypto] secp256k1.normalizeSignatureDER', async (t) => {
    ...
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        const sig = secp256k1.signMessageHashDER(privateKey, hash);
        const malleated = secp256k1.malleateSignatureDER(sig);
        t.deepEqual(secp256k1.normalizeSignatureDER(malleated), new Uint8Array(secp256k1Node.signatureExport(secp256k1Node.signatureNormalize(secp256k1Node.signatureImport(malleated)))));
    });
    t.notThrows(() => {
        fc.assert(malleateThenNormalizeEqualsInitial);
        fc.assert(equivalentToSecp256k1Node);
...
test('[crypto] secp256k1.recoverPublicKeyCompressed', (t) => {
    t.deepEqual(internalSecp256k1.recoverPublicKeyCompressed(sigCompact, sigRecovery, messageHash), pubkeyCompressed);
    t.deepEqual(internalSecp256k1.recoverPublicKeyCompressed(new Uint8Array(64).fill(255), sigRecovery, messageHash), Secp256k1Error.recoverPublicKeyWithUnparsableSignature);
    const failRecover = 2;
    t.deepEqual(internalSecp256k1.recoverPublicKeyCompressed(sigCompact, failRecover, messageHash), Secp256k1Error.recoverPublicKeyInvalidMaterial);
});
test('[fast-check] [crypto] secp256k1.recoverPublicKeyCompressed', async (t) => {
    ...
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        ...
        t.deepEqual(secp256k1.recoverPublicKeyCompressed(recoverableStuff.signature, recoverableStuff.recoveryId, hash), new Uint8Array(secp256k1Node.ecdsaRecover(recoverableStuff.signature, recoverableStuff.recoveryId, hash, true)));
    });
    t.notThrows(() => {
        fc.assert(equivalentToSecp256k1Node);
    });
});
test('[crypto] secp256k1.recoverPublicKeyUncompressed', (t) => {
    t.deepEqual(internalSecp256k1.recoverPublicKeyUncompressed(sigCompact, sigRecovery, messageHash), pubkeyUncompressed);
});
test('[fast-check] [crypto] secp256k1.recoverPublicKeyUncompressed', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        const recoverableStuff = secp256k1.signMessageHashRecoverableCompact(privateKey, hash);
        t.deepEqual(secp256k1.recoverPublicKeyUncompressed(recoverableStuff.signature, recoverableStuff.recoveryId, hash), new Uint8Array(secp256k1Node.ecdsaRecover(recoverableStuff.signature, recoverableStuff.recoveryId, hash, false)));
    });
    t.notThrows(() => {
        fc.assert(equivalentToSecp256k1Node);
    });
});
test('[crypto] secp256k1.signMessageHashCompact', (t) => {
    t.deepEqual(internalSecp256k1.signMessageHashCompact(privkey, messageHash), sigCompact);
    t.notDeepEqual(internalSecp256k1.signMessageHashCompact(privkey, Uint8Array.of()), sigCompact);
    t.deepEqual(internalSecp256k1.signMessageHashCompact(secp256k1OrderN, messageHash), Secp256k1Error.signWithInvalidPrivateKey);
});
test('[fast-check] [crypto] secp256k1.signMessageHashCompact', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        t.deepEqual(secp256k1.signMessageHashCompact(privateKey, hash), new Uint8Array(secp256k1Node.ecdsaSign(hash, privateKey).signature));
    });
    const equivalentToElliptic = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        const { key } = setupElliptic(privateKey);
...
test('[crypto] secp256k1.signMessageHashDER', (t) => {
    t.deepEqual(internalSecp256k1.signMessageHashDER(privkey, messageHash), sigDER);
    t.notDeepEqual(internalSecp256k1.signMessageHashDER(privkey, Uint8Array.of()), sigDER);
    t.deepEqual(internalSecp256k1.signMessageHashDER(secp256k1OrderN, messageHash), Secp256k1Error.signWithInvalidPrivateKey);
});
test('[fast-check] [crypto] secp256k1.signMessageHashDER', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        t.deepEqual(secp256k1.signMessageHashDER(privateKey, hash), new Uint8Array(secp256k1Node.signatureExport(secp256k1Node.ecdsaSign(hash, privateKey).signature)));
    });
    const equivalentToElliptic = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        const { key } = setupElliptic(privateKey);
...
test('[crypto] secp256k1.signMessageHashRecoverableCompact', (t) => {
    const recoverableStuff = internalSecp256k1.signMessageHashRecoverableCompact(privkey, messageHash);
    t.is(recoverableStuff.recoveryId, sigRecovery);
    t.deepEqual(recoverableStuff.signature, sigCompact);
    t.deepEqual(internalSecp256k1.signMessageHashRecoverableCompact(secp256k1OrderN, messageHash), Secp256k1Error.signWithInvalidPrivateKey);
});
...
test('[crypto] secp256k1.signatureCompactToDER', (t) => {
    t.deepEqual(internalSecp256k1.signatureCompactToDER(sigCompact), sigDER);
});
test('[fast-check] [crypto] secp256k1.signatureCompactToDER', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const reversesCompress = fc.property(fcValidPrivateKey, (privateKey) => {
        const pubkeyU = secp256k1.derivePublicKeyUncompressed(privateKey);
        t.deepEqual(pubkeyU, secp256k1.uncompressPublicKey(secp256k1.compressPublicKey(pubkeyU)));
    });
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        const sig = secp256k1.signMessageHashCompact(privateKey, hash);
        t.deepEqual(new Uint8Array(secp256k1Node.signatureExport(sig)), secp256k1.signatureCompactToDER(sig));
    });
    t.notThrows(() => {
...
test('[crypto] secp256k1.signatureDERToCompact', (t) => {
    t.deepEqual(internalSecp256k1.signatureDERToCompact(sigDER), sigCompact);
    const sigDERWithBrokenEncoding = sigDER.slice().fill(0, 0, 1);
    t.deepEqual(internalSecp256k1.signatureDERToCompact(sigDERWithBrokenEncoding), Secp256k1Error.unparsableSignature);
});
test('[fast-check] [crypto] secp256k1.signatureDERToCompact', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), (privateKey, hash) => {
        const sig = secp256k1.signMessageHashDER(privateKey, hash);
        t.deepEqual(new Uint8Array(secp256k1Node.signatureImport(sig)), secp256k1.signatureDERToCompact(sig));
    });
    t.notThrows(() => {
        fc.assert(equivalentToSecp256k1Node);
    });
});
test('[crypto] secp256k1.uncompressPublicKey', (t) => {
    t.deepEqual(internalSecp256k1.uncompressPublicKey(pubkeyCompressed), pubkeyUncompressed);
    t.deepEqual(internalSecp256k1.uncompressPublicKey(new Uint8Array(33)), Secp256k1Error.unparsablePublicKey);
});
test('[fast-check] [crypto] secp256k1.uncompressPublicKey', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, (privateKey) => {
        const pubkeyC = secp256k1.derivePublicKeyCompressed(privateKey);
        t.deepEqual(new Uint8Array(secp256k1Node.publicKeyConvert(pubkeyC, false)), secp256k1.uncompressPublicKey(pubkeyC));
    });
    t.notThrows(() => {
        fc.assert(equivalentToSecp256k1Node);
    });
});
test('[crypto] secp256k1.validatePrivateKey', (t) => {
    t.true(internalSecp256k1.validatePrivateKey(privkey));
    t.false(internalSecp256k1.validatePrivateKey(secp256k1OrderN));
});
test('[fast-check] [crypto] secp256k1.validatePrivateKey', async (t) => {
    ...
    const equivalentToSecp256k1Node = fc.property(fc
        .array(fc.integer({ max: 255, min: 0 }), {
        maxLength: theRest,
        minLength: theRest,
    })
...
test('[crypto] secp256k1.validatePublicKey', (t) => {
    t.true(internalSecp256k1.validatePublicKey(pubkeyUncompressed));
    t.true(internalSecp256k1.validatePublicKey(pubkeyCompressed));
    t.false(internalSecp256k1.validatePublicKey(invalidPubkeyCompressed));
});
test.todo('[fast-check] [crypto] secp256k1.validatePublicKey');
test('[crypto] secp256k1.verifySignatureCompact', (t) => {
    t.true(internalSecp256k1.verifySignatureCompact(sigCompactHighS, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureCompact(sigCompactHighS, pubkeyCompressed, Uint8Array.of()));
    t.true(internalSecp256k1.verifySignatureCompact(sigCompactHighS, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureCompact(Uint8Array.of(), pubkeyCompressed, messageHash));
});
test('[fast-check] [crypto] secp256k1.verifySignatureCompact', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), fc.boolean(), fc.boolean(), (privateKey, message, compressed, invalidate) => {
        ...
        return (secp256k1Node.ecdsaVerify(testSig, message, pub) ===
            secp256k1.verifySignatureCompact(malleated, pub, message));
    });
    const equivalentToElliptic = fc.property(fcValidPrivateKey, fcUint8Array32(), fc.boolean(), fc.boolean(), (privateKey, message, compressed, invalidate) => {
...
test('[crypto] secp256k1.verifySignatureCompactLowS', (t) => {
    t.true(internalSecp256k1.verifySignatureCompactLowS(sigCompact, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureCompactLowS(Uint8Array.of(), pubkeyCompressed, messageHash));
    t.true(internalSecp256k1.verifySignatureCompactLowS(sigCompact, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureCompactLowS(sigCompact, pubkeyCompressed, Uint8Array.of()));
});
...
test('[crypto] secp256k1.verifySignatureDER', (t) => {
    t.true(internalSecp256k1.verifySignatureDER(sigDERHighS, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureDER(Uint8Array.of(), pubkeyCompressed, messageHash));
    t.true(internalSecp256k1.verifySignatureDER(sigDERHighS, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureDER(sigDERHighS, pubkeyCompressed, Uint8Array.of()));
    t.false(internalSecp256k1.verifySignatureDER(Uint8Array.of(), pubkeyCompressed, messageHash));
    t.true(internalSecp256k1.verifySignatureDER(sigDERHighS, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureDER(sigDERHighS, pubkeyCompressed, Uint8Array.of()));
});
test('[crypto] secp256k1.verifySignatureDERLowS', (t) => {
    t.true(internalSecp256k1.verifySignatureDERLowS(sigDER, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureDERLowS(Uint8Array.of(), pubkeyCompressed, messageHash));
    t.true(internalSecp256k1.verifySignatureDERLowS(sigDER, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureDERLowS(sigDER, pubkeyCompressed, Uint8Array.of()));
    t.false(internalSecp256k1.verifySignatureDERLowS(Uint8Array.of(), pubkeyCompressed, messageHash));
    t.true(internalSecp256k1.verifySignatureDERLowS(sigDER, pubkeyCompressed, messageHash));
    t.false(internalSecp256k1.verifySignatureDERLowS(sigDER, pubkeyCompressed, Uint8Array.of()));
    const pubkeyWithBrokenEncoding = pubkeyCompressed.slice().fill(0, 0, 1);
    t.false(internalSecp256k1.verifySignatureDERLowS(sigDER, pubkeyWithBrokenEncoding, messageHash));
    const sigDERWithBrokenEncoding = sigDER.slice().fill(0, 0, 1);
    t.false(internalSecp256k1.verifySignatureDERLowS(sigDERWithBrokenEncoding, pubkeyCompressed, messageHash));
    const sigDERWithBadSignature = sigDER.slice().fill(0, 6, 7);
    t.false(internalSecp256k1.verifySignatureDERLowS(sigDERWithBadSignature, pubkeyCompressed, messageHash));
});
test('[fast-check] [crypto] secp256k1.verifySignatureDERLowS', async (t) => {
    ...
    const equivalentToSecp256k1Node = fc.property(fcValidPrivateKey, fcUint8Array32(), fc.boolean(), fc.boolean(), (privateKey, message, compressed, invalidate) => {
        ...
        return (secp256k1Node.ecdsaVerify(imported, message, pub) ===
...
test('[crypto] secp256k1.signMessageHashSchnorr', (t) => {
    t.deepEqual(internalSecp256k1.signMessageHashSchnorr(privkey, schnorrMsgHash), sigSchnorr);
    t.notDeepEqual(internalSecp256k1.signMessageHashSchnorr(Uint8Array.of(), schnorrMsgHash), sigSchnorr);
    t.deepEqual(internalSecp256k1.signMessageHashSchnorr(secp256k1OrderN, schnorrMsgHash), Secp256k1Error.signWithInvalidPrivateKey);
});
test('[fast-check] [crypto] secp256k1.signMessageHashSchnorr', async (t) => {
    const secp256k1 = await secp256k1Promise;
    const createsValidSignatures = fc.property(fcValidPrivateKey, fcUint8Array32(), fc.boolean(), (privateKey, hash, invalidate) => {
        const publicKey = secp256k1.derivePublicKeyCompressed(privateKey);
        const signature = secp256k1.signMessageHashSchnorr(privateKey, hash);
        t.is(secp256k1.verifySignatureSchnorr(invalidate ? signature : signature.fill(0), publicKey, hash), invalidate);
    });
    t.notThrows(() => {
        fc.assert(createsValidSignatures);
    });
});
test('[crypto] secp256k1.verifySignatureSchnorr', (t) => {
    t.true(internalSecp256k1.verifySignatureSchnorr(sigSchnorr, pubkeyCompressed, schnorrMsgHash));
    t.false(internalSecp256k1.verifySignatureSchnorr(Uint8Array.of(), pubkeyCompressed, schnorrMsgHash));
    const pubkeyWithBrokenEncoding = pubkeyCompressed.slice().fill(0, 0, 1);
    t.false(internalSecp256k1.verifySignatureSchnorr(sigSchnorr, pubkeyWithBrokenEncoding, schnorrMsgHash));
    const sigSchnorrWithBadSignature = sigSchnorr.slice().fill(0, 6, 7);
    t.false(internalSecp256k1.verifySignatureSchnorr(sigSchnorrWithBadSignature, pubkeyCompressed, schnorrMsgHash));
    ...
    t.is(internalSecp256k1.verifySignatureSchnorr(sig1, pk1, msg1), true);
    /* Test vector 2 */
    // prettier-ignore
    ...
    t.is(internalSecp256k1.verifySignatureSchnorr(sig2, pk2, msg2), true);
    ...
    t.is(internalSecp256k1.verifySignatureSchnorr(sig6, pk6, msg6), false);
});
