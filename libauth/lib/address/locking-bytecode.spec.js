...
import { addressContentsToLockingBytecode, Base58AddressError, Base58AddressFormatVersion, base58AddressToLockingBytecode, CashAddressDecodingError, CashAddressEncodingError, CashAddressNetworkPrefix, cashAddressToLockingBytecode, hexToBin, lockingBytecodeToAddressContents, lockingBytecodeToBase58Address, lockingBytecodeToCashAddress, LockingBytecodeType, sha256, } from '../lib.js';
...
test('lockingBytecode <-> AddressContents: unknown', (t) => {
    ...
    t.throws(() => addressContentsToLockingBytecode({
        payload: simpleMath,
...
const cashVectors = test.macro({
    exec: (t, cashAddress, bytecode) => {
        ...
        t.deepEqual(lockingBytecodeToCashAddress(hexToBin(bytecode), 'bitcoincash'), cashAddress, 'lockingBytecodeToCashAddress');
    },
    title: (_, cashAddress) => `cashAddressToLockingBytecode <-> lockingBytecodeToCashAddress: ${cashAddress}`,
});
...
test('cashAddressToLockingBytecode <-> lockingBytecodeToCashAddress: P2PKH', (t) => {
    ...
    t.deepEqual(lockingBytecodeToCashAddress(p2pkh, 'bitcoincash', { tokenSupport: true }), tokenAddress);
...
test('cashAddressToLockingBytecode <-> lockingBytecodeToCashAddress: P2SH20', (t) => {
    ...
    t.deepEqual(lockingBytecodeToCashAddress(p2sh20, 'bitcoincash', { tokenSupport: true }), tokenAddress);
    t.deepEqual(cashAddressToLockingBytecode(address), {
        bytecode: p2sh20,
        options: { tokenSupport: false },
        prefix: 'bitcoincash',
...
test('cashAddressToLockingBytecode <-> lockingBytecodeToCashAddress: P2SH32', (t) => {
    const p2sh32 = hexToBin('aa20000000000000000000000000000012345678900000000000000000000000000087');
    const address = 'bitcoincash:pvqqqqqqqqqqqqqqqqqqqqqqzg69v7ysqqqqqqqqqqqqqqqqqqqqqpkp7fqn0';
    const tokenAddress = 'bitcoincash:rvqqqqqqqqqqqqqqqqqqqqqqzg69v7ysqqqqqqqqqqqqqqqqqqqqqn9alsp2y';
    t.deepEqual(lockingBytecodeToCashAddress(p2sh32, 'bitcoincash'), address);
    t.deepEqual(lockingBytecodeToCashAddress(p2sh32, 'bitcoincash', { tokenSupport: true }), tokenAddress);
    t.deepEqual(cashAddressToLockingBytecode(address), {
        bytecode: p2sh32,
        options: { tokenSupport: false },
        prefix: 'bitcoincash',
...
test('cashAddressToLockingBytecode: error', (t) => {
    ...
    t.deepEqual(cashAddressToLockingBytecode('bitcoincash:dp60yz0ka2g8ut4y3a604czhs2hg5ejj2u6xkulaqj'), `${CashAddressDecodingError.unknownAddressType} Type bit value: 13.`);
});
test('lockingBytecodeToBase58Address: P2PK', (t) => {
    const genesisCoinbase = hexToBin('4104678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef38c4f35504e51ec112de5c384df7ba0b8d578a4c702b6bf11d5fac');
    const genesisPublicKey = hexToBin('04678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef38c4f35504e51ec112de5c384df7ba0b8d578a4c702b6bf11d5f');
    t.deepEqual(lockingBytecodeToBase58Address(genesisCoinbase, 'mainnet', sha256), {
        payload: genesisPublicKey,
        type: LockingBytecodeType.p2pk,
    });
    const genesisCoinbaseCompressed = hexToBin('2103678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb6ac');
    const compressedPublicKey = hexToBin('03678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb6');
    t.deepEqual(lockingBytecodeToBase58Address(genesisCoinbaseCompressed, 'testnet', sha256), {
        payload: compressedPublicKey,
        type: LockingBytecodeType.p2pk,
    });
});
test('base58AddressToLockingBytecode <-> lockingBytecodeToBase58Address: P2PKH', (t) => {
    ...
    t.deepEqual(lockingBytecodeToBase58Address(p2pkh, 'testnet', sha256), addressTestnet);
...
test('base58AddressToLockingBytecode <-> lockingBytecodeToBase58Address: P2SH20', (t) => {
    ...
    t.deepEqual(lockingBytecodeToBase58Address(p2sh20, 'testnet', sha256), addressTestnet);
...
test('base58AddressToLockingBytecode: error', (t) => {
    t.deepEqual(base58AddressToLockingBytecode('bad:address'), Base58AddressError.unknownCharacter);
});
