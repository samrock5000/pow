...
import { assembleBytecode, assembleBytecodeBCH, assembleBytecodeBTC, authenticationInstructionsAreMalformed, decodeAuthenticationInstructions, disassembleAuthenticationInstructionsMaybeMalformed, disassembleBytecode, disassembleBytecodeBCH, disassembleBytecodeBTC, encodeAuthenticationInstruction, encodeAuthenticationInstructions, encodeAuthenticationInstructionsMaybeMalformed, generateBytecodeMap, hexToBin, OpcodesBCH2022, OpcodesBTC, range, } from '../../../lib.js';
...
const disassemble = test.macro({
    exec: (t, input, expected) => {
        t.deepEqual(disassembleAuthenticationInstructionsMaybeMalformed(OpcodesBCH2022, input), expected);
    },
...
const reEncode = test.macro({
    exec: (t, input, expected) => {
        t.deepEqual(encodeAuthenticationInstructionsMaybeMalformed(input), expected);
    },
...
test('disassembleBytecode', (t) => {
    t.deepEqual(disassembleBytecode(TestOpcodes, Uint8Array.from([0, 81, 82, 83, 81, 82, 83])), 'OP_PUSH_EMPTY OP_A OP_B OP_C OP_A OP_B OP_C');
});
test('assembleBytecode', (t) => {
    t.deepEqual(assembleBytecode(generateBytecodeMap(TestOpcodes), 'OP_PUSH_EMPTY OP_A OP_B OP_C OP_A OP_B OP_C'), { bytecode: Uint8Array.from([0, 81, 82, 83, 81, 82, 83]), success: true });
});
...
testProp('[fast-check] disassembleBytecodeBCH <-> assembleBytecodeBCH', [fc.uint8Array({ maxLength: maxBinLength, minLength: 0 })], (t, randomBytecode) => {
    ...
    const instructions = (authenticationInstructionsAreMalformed(parsed)
        ? parsed.slice(0, -1)
        : parsed);
    const minimalPush = instructions.map((instruction) => [OpcodesBCH2022.OP_PUSHDATA_2, OpcodesBCH2022.OP_PUSHDATA_4].includes(instruction.opcode)
...
testProp('[fast-check] disassembleBytecodeBTC <-> assembleBytecodeBTC', [fc.uint8Array({ maxLength: maxBinLength, minLength: 0 })], (t, randomBytecode) => {
    const parsed = decodeAuthenticationInstructions(randomBytecode);
    const instructions = (authenticationInstructionsAreMalformed(parsed)
        ? parsed.slice(0, -1)
        : parsed);
    const minimalPush = instructions.map((instruction) => [OpcodesBTC.OP_PUSHDATA_2, OpcodesBTC.OP_PUSHDATA_4].includes(instruction.opcode)
