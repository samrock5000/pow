...
export const decodeHeader = (bin: Uint8Array): BlockHeader | string => {
  ...
  if (headerRead.position.index !== bin.length) {
    return formatError(
      ...
      `Encoded header ends at index ${headerRead.position.index - 1}, leaving ${
        bin.length - headerRead.position.index
      } remaining bytes.`,
    );
  }
