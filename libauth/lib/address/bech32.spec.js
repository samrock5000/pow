...
import { Bech32DecodingError, bech32PaddedToBin, binToBech32Padded, binToUtf8, BitRegroupingError, decodeBech32, encodeBech32, isBech32CharacterSet, regroupBits, } from '../lib.js';
import { testProp } from '@fast-check/ava';
import fc from 'fast-check';
test('regroupBits', (t) => {
    t.deepEqual(regroupBits({
    ...
    t.deepEqual(regroupBits({ bin: [256], resultWordLength: 5, sourceWordLength: 8 }), BitRegroupingError.integerOutOfRange);
    t.deepEqual(regroupBits({
        allowPadding: false,
...
test('bech32PaddedToBin', (t) => {
    t.deepEqual(bech32PaddedToBin('qqq'), BitRegroupingError.hasDisallowedPadding);
    t.deepEqual(bech32PaddedToBin('qq'), Uint8Array.of(0));
    t.deepEqual(bech32PaddedToBin('tf'), BitRegroupingError.requiresDisallowedPadding);
    t.deepEqual(bech32PaddedToBin('lu'), Uint8Array.of(255));
    // cspell: disable-next-line
    t.deepEqual(bech32PaddedToBin('qqqsyqc'), Uint8Array.of(0, 1, 2, 3));
    t.deepEqual(bech32PaddedToBin('qqq1'), Bech32DecodingError.notBech32CharacterSet);
});
...
testProp('[fast-check] encodeBech32 <-> decodeBech32', [
...
testProp('[fast-check] bech32PaddedToBin <-> binToBech32Padded', [fcUint8Array(0, maxBinLength)], (t, input) => {
    t.deepEqual(binToBech32Padded(bech32PaddedToBin(binToBech32Padded(input))), binToBech32Padded(input));
});
testProp('[fast-check] binToBech32Padded -> isBech32', [fcUint8Array(0, maxBinLength)], (t, input) => t.true(isBech32CharacterSet(binToBech32Padded(input))));
testProp('[fast-check] isBech32: matches round trip results', [fcUint8Array(0, maxBinLength)], (t, input) => {
