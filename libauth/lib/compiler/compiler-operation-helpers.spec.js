...
import { compilerOperationHelperGenerateCoveredBytecode, compilerOperationRequires, stringifyTestVector, } from '../lib.js';
test('attemptCompilerOperations: can skip configuration property check', (t) => {
    t.deepEqual(compilerOperationRequires({
        canBeSkipped: true,
...
test('compilerOperationHelperGenerateCoveredBytecode: empty sourceScriptIds', (t) => {
    ...
    t.deepEqual(result, {
        error: 'Identifier "test" requires a signing serialization, but "coveredBytecode" cannot be determined because the compiler configuration\'s "sourceScriptIds" is empty.',
        status: 'error',
