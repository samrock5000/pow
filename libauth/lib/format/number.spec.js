...
import { bigIntToBinUint256BEClamped, bigIntToBinUint64LE, bigIntToBinUint64LEClamped, bigIntToBinUintLE, bigIntToCompactUint, binToBigIntUint256BE, binToBigIntUint64LE, binToBigIntUintBE, binToBigIntUintLE, binToHex, binToNumberInt16LE, binToNumberInt32LE, binToNumberUint16LE, binToNumberUint32LE, binToNumberUintLE, CompactUintError, compactUintPrefixToSize, compactUintToBigInt, hexToBin, int32SignedToUnsigned, int32UnsignedToSigned, numberToBinInt16LE, numberToBinInt32LE, numberToBinInt32TwosCompliment, numberToBinUint16BE, numberToBinUint16LE, numberToBinUint16LEClamped, numberToBinUint32BE, numberToBinUint32LE, numberToBinUint32LEClamped, numberToBinUintLE, readCompactUint, readCompactUintMinimal, } from '../lib.js';
...
test('bigIntToBinUint64LE', (t) => {
    t.deepEqual(bigIntToBinUint64LE(0n), Uint8Array.from([0, 0, 0, 0, 0, 0, 0, 0]));
    t.deepEqual(bigIntToBinUint64LE(0x01n), Uint8Array.from([0x01, 0, 0, 0, 0, 0, 0, 0]));
    t.deepEqual(bigIntToBinUint64LE(0x12345678n), Uint8Array.from([0x78, 0x56, 0x34, 0x12, 0, 0, 0, 0]));
    t.deepEqual(bigIntToBinUint64LE(BigInt(Number.MAX_SAFE_INTEGER)), Uint8Array.from([255, 255, 255, 255, 255, 255, 31, 0]));
    t.deepEqual(bigIntToBinUint64LE(0xffffffffffffffffn), Uint8Array.from([255, 255, 255, 255, 255, 255, 255, 255]));
});
test('bigIntToBinUint64LE vs. bigIntToBinUint64LEClamped: behavior on overflow', (t) => {
    t.deepEqual(bigIntToBinUint64LE(0x010000000000000000n), bigIntToBinUint64LE(0x010000000000000000n % (0xffffffffffffffffn + 1n)));
    t.deepEqual(bigIntToBinUint64LEClamped(0x010000000000000000n), Uint8Array.from([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]));
});
test('bigIntToBinUint64LE vs. bigIntToBinUint64LEClamped: behavior on negative numbers', (t) => {
    t.deepEqual(bigIntToBinUint64LE(-1n), Uint8Array.from([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]));
    t.deepEqual(bigIntToBinUint64LEClamped(-1n), Uint8Array.from([0, 0, 0, 0, 0, 0, 0, 0]));
});
test('bigIntToCompactUint: larger values return modulo result after opcode', (t) => {
    t.deepEqual(bigIntToCompactUint(0x010000000000000001n), Uint8Array.from([0xff, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]));
});
...
testProp('[fast-check] numberToBinUintLE <-> binToNumberUintLE', [fc.integer({ max: Number.MAX_SAFE_INTEGER, min: 0 })], (t, maxSafeInt) => t.deepEqual(binToNumberUintLE(numberToBinUintLE(maxSafeInt)), maxSafeInt));
...
test('binToBigIntUintBE', (t) => {
    ...
    t.deepEqual(binToBigIntUintBE(Uint8Array.from([0x12, 0x34, 0x56])), 0x123456n);
    t.deepEqual(binToBigIntUintBE(Uint8Array.from([0x12, 0x34, 0x56, 0x78])), 0x12345678n);
    t.deepEqual(binToBigIntUintBE(Uint8Array.from([0x12, 0x34, 0x56, 0x78, 0x90])), 0x1234567890n);
    t.deepEqual(binToBigIntUintBE(Uint8Array.from([0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef])), 0x1234567890abcdefn);
    t.deepEqual(binToBigIntUintBE(Uint8Array.from([0x56, 0x78, 0x90, 0xab, 0xcd, 0xef])), 0x567890abcdefn);
...
test('binToBigIntUintLE', (t) => {
    ...
    t.deepEqual(binToBigIntUintLE(Uint8Array.from([0x56, 0x34, 0x12])), 0x123456n);
    t.deepEqual(binToBigIntUintLE(Uint8Array.from([0x78, 0x56, 0x34, 0x12])), 0x12345678n);
    t.deepEqual(binToBigIntUintLE(Uint8Array.from([0x90, 0x78, 0x56, 0x34, 0x12])), 0x1234567890n);
    t.deepEqual(binToBigIntUintLE(Uint8Array.from([0xef, 0xcd, 0xab, 0x90, 0x78, 0x56, 0x34, 0x12])), 0x1234567890abcdefn);
    t.deepEqual(binToBigIntUintLE(Uint8Array.from([0xab, 0x90, 0x78, 0x56, 0x34, 0x12])), 0x1234567890abn);
...
test('binToBigIntUint64LE', (t) => {
    t.deepEqual(binToBigIntUint64LE(Uint8Array.from([0x78, 0x56, 0x34, 0x12, 0, 0, 0, 0])), 0x12345678n);
    t.deepEqual(binToBigIntUint64LE(Uint8Array.from([0xef, 0xcd, 0xab, 0x89, 0x67, 0x45, 0x23, 0x01])), 0x0123456789abcdefn);
...
test('readCompactUint', (t) => {
    ...
    t.deepEqual(readCompactUint({ bin: Uint8Array.from([253, 0, 0]), index: 0 }), { position: { bin: Uint8Array.from([253, 0, 0]), index: 3 }, result: 0n });
    t.deepEqual(readCompactUint({ bin: Uint8Array.from([254, 0, 0, 0, 0]), index: 0 }), {
        position: { bin: Uint8Array.from([254, 0, 0, 0, 0]), index: 5 },
        result: 0n,
    });
    t.deepEqual(readCompactUint({
    ...
    t.deepEqual(readCompactUint({ bin: Uint8Array.from([253, 253, 0]), index: 0 }), {
        position: { bin: Uint8Array.from([253, 253, 0]), index: 3 },
        result: 253n,
    });
    t.deepEqual(readCompactUint({ bin: Uint8Array.from([]), index: 0 }), CompactUintError.noPrefix);
});
test('readCompactUintMinimal', (t) => {
    ...
    t.deepEqual(readCompactUintMinimal({ bin: Uint8Array.from([253, 1, 0]), index: 0 }), `${CompactUintError.nonMinimal} Value: 1, encoded length: 3, canonical length: 1`);
});
test('compactUintToBigInt', (t) => {
    ...
    t.deepEqual(compactUintToBigInt(Uint8Array.from([253])), 'Error reading CompactUint: insufficient bytes. CompactUint prefix 253 requires at least 3 bytes. Remaining bytes: 1');
    t.deepEqual(compactUintToBigInt(Uint8Array.from([253, 0, 254, 0])), 'Error decoding CompactUint: unexpected bytes after CompactUint. CompactUint ends at index 3, but input includes 4 bytes.');
});
...
test(compactUintVector, '0000ff0000000001000000', 0x0100000000n, 11, 2, 'ff0000000001000000');
...
testProp('[fast-check] bigIntToCompactUint <-> compactUintToBigInt', [fc.bigUintN(64)], (t, uint64) => {
...
testProp('[fast-check] int32UnsignedToSigned <-> int32SignedToUnsigned', [fc.integer({ max: 2 ** 32 - 1, min: 0 })], (t, uint32) => {
