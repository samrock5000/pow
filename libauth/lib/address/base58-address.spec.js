...
import { Base58AddressError, Base58AddressFormatVersion, decodeBase58Address, decodeBase58AddressFormat, decodePrivateKeyWif, encodeBase58Address, encodeBase58AddressFormat, hexToBin, lockingBytecodeToBase58Address, sha256, } from '../lib.js';
...
test('encodeBase58AddressFormat', (t) => {
    ...
    t.deepEqual(encodeBase58AddressFormat(Base58AddressFormatVersion.p2pkh, payload), 
    // cspell: disable-next-line
    '1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62i');
    t.deepEqual(encodeBase58AddressFormat(Base58AddressFormatVersion.p2pkh, payload, sha256), 
    // cspell: disable-next-line
    '1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62i');
});
test('encodeBase58Address', (t) => {
    const payload = hexToBin('76a04053bda0a88bda5177b86a15c3b29f559873');
    t.deepEqual(encodeBase58Address('p2pkh', payload, sha256), 
    // cspell: disable-next-line
    '1BpEi6DfDAUFd7GtittLSdBeYJvcoaVggu');
    t.deepEqual(encodeBase58Address('p2pkhTestnet', payload, sha256), 
    // cspell: disable-next-line
    'mrLC19Je2BuWQDkWSTriGYPyQJXKkkBmCx');
    t.deepEqual(encodeBase58Address('p2pkhCopayBCH', payload), 
    // cspell: disable-next-line
    'CTH8H8Zj6DSnXFBKQeDG28ogAS92iS16Bp');
    t.deepEqual(encodeBase58Address('p2sh20', payload, sha256), 
    // cspell: disable-next-line
    '3CWFddi6m4ndiGyKqzYvsFYagqDLPVMTzC');
    t.deepEqual(encodeBase58Address('p2sh20Testnet', payload), 
    // cspell: disable-next-line
    '2N44ThNe8NXHyv4bsX8AoVCXquBRW94Ls7W');
    t.deepEqual(encodeBase58Address('p2sh20CopayBCH', payload, sha256), 
    // cspell: disable-next-line
...
test('decodeBase58AddressFormat', (t) => {
    ...
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58AddressFormat('1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62i'), { payload, version: Base58AddressFormatVersion.p2pkh });
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58AddressFormat('1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62i', sha256), { payload, version: Base58AddressFormatVersion.p2pkh });
});
test('decodeBase58Address', (t) => {
    const payload = hexToBin('65a16059864a2fdbc7c99a4723a8395bc6f188eb');
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58Address('1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62i'), { payload, version: Base58AddressFormatVersion.p2pkh });
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58Address('1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62i', sha256), { payload, version: Base58AddressFormatVersion.p2pkh });
});
const maxUint8Number = 255;
const maxBinLength = 100;
testProp('[fast-check] encodeBase58Address <-> decodeBase58Address', [
...
test('decodeBase58AddressFormat: errors', (t) => {
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58AddressFormat('1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62a'), Base58AddressError.invalidChecksum);
    t.deepEqual(decodeBase58AddressFormat('1234'), Base58AddressError.tooShort);
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58AddressFormat('1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62I'), Base58AddressError.unknownCharacter);
});
test('decodeBase58Address: errors', (t) => {
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58Address('6PfDNQxJdsBx7K4r9kMrRBZSa2NZKVNUZn'), Base58AddressError.unknownAddressVersion);
    t.deepEqual(
    // cspell: disable-next-line
...
test('Base58Address Valid Vectors (from C++ implementation – includes WIF vectors)', (t) => {
    ...
    validVectors.forEach((vectors) => {
        ...
        if (meta.isPrivkey) {
            ...
            t.deepEqual(lockingBytecodeToBase58Address(hexToBin(lockingBytecode), testnet ? 'testnet' : 'mainnet', sha256), base58Address);
        }
    });
});
