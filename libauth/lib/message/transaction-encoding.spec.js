...
import { decodeTransactionCommon, encodeTokenPrefix, encodeTransactionCommon, hashTransaction, hashTransactionP2pOrder, hashTransactionUiOrder, hexToBin, readLockingBytecodeWithPrefix, readTokenAmount, readTokenPrefix, readTransaction, readTransactionCommon, readTransactionNonTokenAware, readTransactionOutputNonTokenAware, readTransactionOutputsNonTokenAware, sha256, } from '../lib.js';
...
test('decodeTransaction', (t) => {
     ...
     */
    const tx = hexToBin('');
    t.deepEqual(decodeTransactionCommon(tx), {
        inputs: [
            {
                outpointIndex: 3,
                outpointTransactionHash: hexToBin('1eca9d89dfe01a50627c0d1e8befbe005234aefee08dfec1a380009b46f9162d'),
                sequenceNumber: 4294967295,
                unlockingBytecode: hexToBin('6a0065525365'),
            },
            {
                outpointIndex: 3,
                outpointTransactionHash: hexToBin('fd1718e192d832ce48fca0508084186bdd410f6a0b5d9bd8ddab8b5e339a4ad1'),
                sequenceNumber: 4079810175,
                unlockingBytecode: hexToBin('63acac0053520052'),
            },
            {
                outpointIndex: 2,
                outpointTransactionHash: hexToBin('e023f2ebc7f212764f6e15556b26b586801a12503930e6d5ae0e184e06d930ad'),
                sequenceNumber: 4294967295,
                unlockingBytecode: hexToBin('00'),
            },
            {
                outpointIndex: 1,
                outpointTransactionHash: hexToBin('ba88ff9d44fd01d1c0c0a5b416e00e79b4824606a3a70f163159b5ce3aca7362'),
...
test('decode and encode transaction', (t) => {
    ...
    t.deepEqual(hexToBin(tx), encodeTransactionCommon(decodeTransactionCommon(hexToBin(tx))));
});
test('decodeTransaction: invalid', (t) => {
    t.deepEqual(decodeTransactionCommon(hexToBin('00')), 'Error reading transaction. Error reading Uint32LE: requires 4 bytes. Remaining bytes: 1');
});
test('hashTransaction, hashTransactionUiOrder, hashTransactionP2pOrder', (t) => {
    const tx = '';
    const txId = '67adfe4f2b374e770584d2d0beb8eacc7b29287a47d2ffe511fa81f48e0ec4fb';
    const halTx = '';
    const halTxId = 'f4184fc596403b9d638783cf57adfe4c75c605f6356fbc91338530e9831e9e16';
    ...
    t.deepEqual(hashTransactionP2pOrder(hexToBin(tx), sha256), hexToBin(txId).reverse());
    t.deepEqual(hashTransaction(hexToBin(halTx)), halTxId);
    t.deepEqual(hashTransactionUiOrder(hexToBin(halTx)), hexToBin(halTxId));
    t.deepEqual(hashTransactionUiOrder(hexToBin(halTx), sha256), hexToBin(halTxId));
    t.deepEqual(hashTransactionP2pOrder(hexToBin(halTx)), hexToBin(halTxId).reverse());
    t.deepEqual(hashTransactionP2pOrder(hexToBin(halTx), sha256), hexToBin(halTxId).reverse());
});
test('CashTokens: token-prefix-valid.json', (t) => {
    Object.values(tokenPrefixValidJson)
        .filter((item) => !Array.isArray(item))
        .forEach((vector, index) => {
        const { prefix: prefixHex, data } = vector;
        ...
        if (decoded.position.bin.length !== decoded.position.index) {
            return t.fail(`${failMessage} Prefix "${prefixHex}" includes unexpected bytes. Bytes remaining after read: ${decoded.position.bin.length - decoded.position.index}, `);
        }
...
test('CashTokens: token-prefix-invalid.json', (t) => {
    Object.values(tokenPrefixInvalidJson)
        .filter((item) => !Array.isArray(item))
        .forEach((vector, index) => {
        const { prefix: prefixHex, error } = vector;
        const failMessage = `Failure on test vector #${index}: "${prefixHex}"`;
        const expectedPrefix = hexToBin(prefixHex);
        const decoded = readTokenPrefix({ bin: expectedPrefix, index: 0 });
...
test.todo('[fast-check] encodeTransaction <-> decodeTransaction/decodeTransactionUnsafe; encodeTransactionBCH <-> decodeTransactionBCH/decodeTransactionUnsafeBCH, encodeTransactionCommon <-> decodeTransactionCommon/decodeTransactionUnsafeCommon');
...
//# sourceMappingURL=transaction-encoding.spec.js.map