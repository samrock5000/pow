...
import { assembleBitcoinSatoshiScript, createTestAuthenticationProgramBCH, createVirtualMachineXEC, disassembleBytecodeBCH, stackItemIsTruthy, stringify, stringifyDebugTraceSummary, summarizeDebugTrace, } from '../../../lib.js';
...
const tests = Object.values(scriptTests)
    ...
    const valueSatoshis = typeof expectation[0] === 'string'
        ? 0
...
pendingTests.map((expectation) => {
    const description = `[script_tests] ${expectation.testIndex}/${pendingTests.length} - "${elide(expectation.unlockingBytecodeText, 100)}" | "${elide(expectation.lockingBytecodeText, 100)}" ${expectation.expectedError === false ? 'passes' : expectation.expectedError} ${expectation.message === undefined ? '' : ` # ${expectation.message}`}`;
    ...
    (t) => {
        const unlockingBytecode = assembleBitcoinSatoshiScript(expectation.unlockingBytecodeText);
        const lockingBytecode = assembleBitcoinSatoshiScript(expectation.lockingBytecodeText);
        ...
        if (!pass) {
            ...
            t.log('debug:', stringifyDebugTraceSummary(summarizeDebugTrace(vm.debug(program))));
...
//# sourceMappingURL=xec.script-tests.spec.js.map