...
import { base58ToBin, BaseConversionError, binToBase58, createBaseConverter, hexToBin, range, utf8ToBin, } from '../lib.js';
...
testProp('[fast-check] base2.encode <-> base2.encode', [fc.uint8Array({ maxLength: 100, minLength: 0 })], (t, input) => t.deepEqual(base2.encode(base2.decode(base2.encode(input))), base2.encode(input)));
testProp('[fast-check] base16.encode <-> base16.encode', [fc.uint8Array({ maxLength: 100, minLength: 0 })], (t, input) => t.deepEqual(base16.encode(base16.decode(base16.encode(input))), base16.encode(input)));
const base26 = createBaseConverter(
// cspell: disable-next-line
'abcdefghijklmnopqrstuvwxyz');
testProp('[fast-check] base26.encode <-> base26.encode', [fc.uint8Array({ maxLength: 100, minLength: 0 })], (t, input) => t.deepEqual(base26.encode(base26.decode(base26.encode(input))), base26.encode(input)));
const base42 = createBaseConverter('123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghi');
testProp('[fast-check] base42.encode <-> base42.encode', [fc.uint8Array({ maxLength: 100, minLength: 0 })], (t, input) => t.deepEqual(base42.encode(base42.decode(base42.encode(input))), base42.encode(input)));
testProp('[fast-check] binToBase58 <-> base58ToBin', [fc.uint8Array({ maxLength: 100, minLength: 0 })], (t, input) => t.deepEqual(binToBase58(base58ToBin(binToBase58(input))), binToBase58(input)));
