...
import { compilerOperationAddressData, compileScript, compileScriptContents, describeExpectedInput, hexToBin, stringifyTestVector, } from '../lib.js';
test('compileScript: unprovided ID', (t) => {
    t.deepEqual(compileScript('test', {}, { scripts: { typo: '1' } }), {
        errorType: 'parse',
        errors: [
            {
                error: 'No script with an ID of "test" was provided in the compiler configuration.',
                range: {
                    endColumn: 0,
                    endLineNumber: 0,
                    startColumn: 0,
...
test('compileScript: clean errors on unexpected input', (t) => {
    t.deepEqual(compileScript('t', {}, { scripts: { t: 'te$t' } }), {
        ...
        errors: [
            {
                error: "Encountered unexpected input while parsing script. Expected the opening parenthesis of this evaluation ('(').",
                range: {
                    endColumn: 4,
                    endLineNumber: 1,
                    startColumn: 4,
    ...
    t.deepEqual(compileScript('t', {}, { scripts: { t: '<$(<1> <2> OP_ADD >' } }), {
        errorType: 'parse',
        errors: [
            {
    ...
    t.deepEqual(compileScript('t', {}, { scripts: { t: '"incomplete' } }), {
        ...
        errors: [
            {
                error: 'Encountered unexpected input while parsing script. Expected a closing double quote (").',
                range: {
                    endColumn: 12,
                    endLineNumber: 1,
                    startColumn: 12,
...
test('compileScriptContents: empty string', (t) => {
    t.deepEqual(compileScriptContents({
...
test('describeExpectedInput: handles EOF', (t) => {
    t.deepEqual(describeExpectedInput(['one thing', 'EOF']), 'Encountered unexpected input while parsing script. Expected one thing or the end of the script.');
});
test('describeExpectedInput: handles 2 options', (t) => {
    t.deepEqual(describeExpectedInput(['one thing', 'another thing']), 'Encountered unexpected input while parsing script. Expected one thing or another thing.');
});
...
test('compileScript: variable and script inclusion', (t) => {
    ...
    t.deepEqual(comp, {
        ...
        success: true,
    }, stringifyTestVector(comp));
});
test('compileScript: comments', (t) => {
    t.deepEqual(compileScript('t', {}, { scripts: { t: '// single-line\n  /* multi-\nline */' } }), {
        bytecode: Uint8Array.of(),
        parse: {
            end: {
                column: 8,
