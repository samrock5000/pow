...
export const testHashFunction = ({ abcHash, libauthHash, getEmbeddedBinary, hashFunctionName, instantiate, instantiateBytes, nodeJsAlgorithm, testHash, }) => {
    const binary = getEmbeddedBinary();
    test(`[crypto] ${hashFunctionName} getEmbeddedBinary returns the proper binary`, (t) => {
        const path = join(new URL('.', import.meta.url).pathname, '..', 'bin', hashFunctionName, `${hashFunctionName}.wasm`);
    ...
    test(`[crypto] ${hashFunctionName} incremental hashing`, async (t) => {
        const hashFunction = await instantiate();
        t.deepEqual(hashFunction.final(hashFunction.update(hashFunction.update(hashFunction.update(hashFunction.init(), utf8ToBin('a')), utf8ToBin('b')), utf8ToBin('c'))), abcHash);
        t.deepEqual(hashFunction.final(hashFunction.update(hashFunction.init(), utf8ToBin('test'))), testHash);
        t.deepEqual(hashFunction.final(hashFunction.update(hashFunction.update(hashFunction.init(), utf8ToBin('lib')), utf8ToBin('auth'))), libauthHash);
        const equivalentToSinglePass = fc.property(fcUint8Array(1, testLength), fc.integer({ max: testLength, min: 1 }), (message, chunkSize) => {
