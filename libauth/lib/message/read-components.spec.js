...
import { bigIntToBinUint64LE, hexToBin, numberToBinUint32LE, readBytes, ReadBytesError, readUint32LE, ReadUint32LEError, readUint64LE, ReadUint64LEError,
/*
...
test('readBytes', (t) => {
    ...
    t.deepEqual(readBytes(1)({ bin, index: 4 }), `${ReadBytesError.insufficientLength} Bytes requested: 1; remaining bytes: 0`);
...
test('readUint32LE', (t) => {
    ...
    t.deepEqual(readUint32LE({ bin: hexToBin('0001'), index: 0 }), `${ReadUint32LEError.insufficientBytes} Remaining bytes: 2`);
});
test('readUint64LE', (t) => {
    const bin = hexToBin('00010203ffffffff');
    const num = 0xffffffff03020100n;
    const result = {
    ...
    t.deepEqual(bin, mutates);
    t.deepEqual(readUint64LE({ bin: hexToBin('00010203'), index: 0 }), `${ReadUint64LEError.insufficientBytes} Remaining bytes: 4`);
});
test.todo('readCompactUintPrefixedBin');
test.todo('readRemainingBytes');
//# sourceMappingURL=read-components.spec.js.map