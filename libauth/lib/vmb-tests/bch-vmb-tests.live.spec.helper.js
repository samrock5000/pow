...
import { binsAreEqual, binToHex, cashAddressToLockingBytecode, createCompilerBCH, createVirtualMachineBCH2022, decodeTransactionUnsafe, deriveHdPrivateNodeFromSeed, encodeHdPrivateKey, encodeTransaction, hdPrivateKeyToIdentifier, hdPrivateKeyToP2pkhAddress, hdPrivateKeyToP2pkhLockingBytecode, hexToBin, isHex, lockingBytecodeToBase58Address, lockingBytecodeToCashAddress, stringify, walletTemplateToCompilerConfiguration, } from '../lib.js';
const usageInfo = `
Usage:
...
if (arg1 === 'private') {
    if (arg2 !== undefined && !isHex(arg2)) {
        console.log('Seed must be hex encoded. To use a random seed, omit this option.');
        process.exit(1);
    }
    const seed = arg2 === undefined ? randomBytes(defaultSeedLength) : hexToBin(arg2);
    const node = deriveHdPrivateNodeFromSeed(seed);
    if (!node.valid) {
        console.log(`Tell everyone you found an invalid HD seed 🤯: ${binToHex(seed)}`);
        process.exit(1);
    }
...
if (arg1 === 'address') {
    ...
    const legacyAddress = lockingBytecodeToBase58Address(bytecode, 'mainnet');
    const copayAddress = lockingBytecodeToBase58Address(bytecode, 'copayBCH');
    console.log(`
...
if (arg2 === undefined || arg3 === undefined) {
    console.log('Required arguments for "generate": <hd_private_key> <transaction_hex>');
    console.log(usageInfo);
    process.exit(1);
}
...
const fundingAddress = lockingBytecodeToCashAddress(fundingLockingBytecode, 'bitcoincash');
const fundingUtxoIndex = fundingTransaction.outputs.findIndex((output) => binsAreEqual(output.lockingBytecode, fundingLockingBytecode));
if (fundingUtxoIndex === -1) {
    console.log(`The provided funding transaction does not have an output which pays to address index ${fundingAddressIndex} of the provided HD private key. Is this the correct transaction?
...
if (typeof outputLockingBytecode === 'string') {
    console.log(`The output address "${outputAddress}" is invalid: ${outputLockingBytecode}`);
    process.exit(1);
}
...
if (typeof setupTx.scenario === 'string') {
    console.log(`Error while generating setupTransaction.scenario - ${setupTx.scenario}`);
    process.exit(1);
}
...
//# sourceMappingURL=bch-vmb-tests.live.spec.helper.js.map