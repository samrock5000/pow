import { createVirtualMachineBCH2022, createVirtualMachineBCH2023, createVirtualMachineBCHCHIPs, hexToBin, isPayToScriptHash20, readTransaction, readTransactionNonTokenAware, readTransactionOutputs, readTransactionOutputsNonTokenAware, stringify, stringifyDebugTraceSummary, summarizeDebugTrace, } from '../lib.js';
...
const isVm = (vmId) => Object.keys(vms).includes(vmId);
// cspell:ignore crzlx
const usageInfo = `
...
const vm = vms[vmId];
const nonTokenAware = vmId === 'bch_2022_nonstandard' || vmId === 'bch_2022_standard';
const testDefinition = vmbTestsBCHJson.find(([shortId]) => shortId === testId);
if (testDefinition === undefined) {
    console.log(`Error: the test ID "${testId}" is unknown.\n${usageInfo}`);
    process.exit(1);
}
const [shortId, testDescription, unlockingScriptAsm, redeemOrLockingScriptAsm, txHex, sourceOutputsHex, testSets, inputIndex,] = testDefinition;
const testedIndex = inputIndex ?? 0;
const { result: transaction } = (nonTokenAware ? readTransactionNonTokenAware : readTransaction)({ bin: hexToBin(txHex), index: 0 });
const { result: sourceOutputs } = (nonTokenAware ? readTransactionOutputsNonTokenAware : readTransactionOutputs)({ bin: hexToBin(sourceOutputsHex), index: 0 });
...
const failingIndex = typeof result === 'string'
    ? /evaluating input index (?<index>\d+)/u.exec(result)?.groups?.['index']
    : undefined;
const unexpectedFailingIndexDebugTrace = failingIndex !== undefined && Number(failingIndex) !== testedIndex
    ? vm.debug({
    ...
    : undefined;
const isP2sh20 = isPayToScriptHash20(sourceOutputs[testedIndex].lockingBytecode);
const verbose = `
...
Result: ${result === true ? 'Success' : `Failed: ${result}`}${unexpectedFailingIndexDebugTrace === undefined
    ? ''
    : `
...
${stringifyDebugTraceSummary(summarizeDebugTrace(unexpectedFailingIndexDebugTrace))}
`}
Evaluation at index ${testedIndex}:

${stringifyDebugTraceSummary(summarizeDebugTrace(debugResult))}
`);
//# sourceMappingURL=bch-vmb-test.spec.helper.js.map