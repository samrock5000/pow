...
import { binToHex, decodeTransactionUnsafeCommon, encodeSigningSerializationBCH, generateSigningSerializationComponentsBCH, hash256, hexToBin, isLegacySigningSerialization, numberToBinInt32TwosCompliment, } from '../../../lib.js';
...
pendingTests.map((expectation, currentTest) => {
    test.skip(`[signing-serialization tests] sighash.json ${currentTest}/${pendingTests.length} (#${expectation.testIndex})`, (t) => {
        const tx = decodeTransactionUnsafeCommon(hexToBin(expectation.transactionHex));
