...
test('[crypto] deriveHdPrivateNodeFromSeed', (t) => {
    const valid = {
        ...
        privateKey: hexToBin('330fd355e141910d33bbe84c369b87a209dd18b81095912be766b2b5a9d72bc4'),
        valid: true,
    };
    const invalid = {
        chainCode: hexToBin('18aab7e9ef169f3029d93651d0c85303cbcc2ac559ccd04c324a2e678ef26dc9'),
        childIndex: 0,
        depth: 0,
        invalidPrivateKey: hexToBin('330fd355e141910d33bbe84c369b87a209dd18b81095912be766b2b5a9d72bc4'),
        parentFingerprint: hexToBin('00000000'),
        valid: false,
    };
...
test('[crypto] deriveHdPrivateNodeIdentifier', (t) => {
    const { node } = decodeHdPrivateKey(xprv);
    t.deepEqual(deriveHdPrivateNodeIdentifier(node), hexToBin('15c918d389673c6cd0660050f268a843361e1111'));
    t.deepEqual(deriveHdPrivateNodeIdentifier(node, crypto), hexToBin('15c918d389673c6cd0660050f268a843361e1111'));
});
test('[crypto] deriveHdPublicNodeIdentifier', (t) => {
    const { node } = decodeHdPublicKey(xpub);
    t.deepEqual(deriveHdPublicNodeIdentifier(node), hexToBin('15c918d389673c6cd0660050f268a843361e1111'));
    t.deepEqual(deriveHdPublicNodeIdentifier(node, crypto), hexToBin('15c918d389673c6cd0660050f268a843361e1111'));
});
test('[crypto] decodeHdKey', (t) => {
    ...
    t.deepEqual(decodeHdKey(xpub, crypto), {
        node: {
            ...
            depth: 0,
            parentFingerprint: hexToBin('00000000'),
            publicKey: hexToBin('02be99138b48b430a8ee40bf8b56c8ebc584c363774010a9bfe549a87126e61746'),
        },
...
test('[crypto] decodeHdKey: errors', (t) => {
    ...
    t.deepEqual(decodeHdKey('xpub661MyMwAqRbcEngHLkoUWCKMBMGeqdqCGkqtzrHaTZub9ALw2oRfHA6fQP5n5X9VHStaNTBYomkSb8BFhUGavwD3RG1qvMkEKceTavTp2Ta', crypto), HdKeyDecodingError.invalidChecksum);
});
...
test('[crypto] decodeHdPrivateKey: errors', (t) => {
    ...
    t.deepEqual(decodeHdPrivateKey('1111111111111FF9QeH94hg7KAjgjUqkHUqbrw5wWQLoRNfRhB4cHUDCJxx2HfNb5qDiAjpbKjXeLJSknuzDmja42174H9Es1XbY24sZts9'), HdKeyDecodingError.unknownVersion);
    ...
    t.deepEqual(decodeHdPrivateKey(xprvWith0FilledKey), HdKeyDecodingError.invalidPrivateNode);
    ...
    t.deepEqual(decodeHdPrivateKey(xprvWith255FilledKey), HdKeyDecodingError.invalidPrivateNode);
    t.deepEqual(decodeHdPrivateKey('xprv9s21ZrQH143K2JbpEjGU94NcdKSASB7LuXvJCTsxuENcGN1nVG7QjMnBhegPMjkj1oGSFcmBkMX3xdwcMy6NSgrHvmqJptpUW5xGjg7kifZ'), HdKeyDecodingError.missingPrivateKeyPaddingByte);
});
...
test('[crypto] decodeHdPublicKey: errors', (t) => {
    ...
    t.deepEqual(decodeHdPublicKey('1111111111111FF9QeH94hg7KAjgjUqkHUqbrw5wWQLoRNfRhB4cHUDCJxx2HfNb5qDiAjpbKjXeLJSknuzDmja42174H9Es1XbY24sZts9'), HdKeyDecodingError.unknownVersion);
});
test('[crypto] encodeHdPrivateKey', (t) => {
    t.deepEqual(encodeHdPrivateKey({
        network: 'mainnet',
        node: {
            chainCode: hexToBin('18aab7e9ef169f3029d93651d0c85303cbcc2ac559ccd04c324a2e678ef26dc9'),
            childIndex: 0,
...
test('[crypto] deriveHdPrivateNodeChild', (t) => {
    ...
    const hardenedIndex0Child = decodeHdPrivateKey('xprv9u4S6TaiPQaF7FS24QFpRP6hjff4jXNwwYTkVNC6f6YzHj2a6G28aRN1D6Az82SxMeBivpVS3gDDXyQiu3RANTqWy34Zxi9JN76zSwkjqPF');
    const index1GrandChild = decodeHdPrivateKey('xprv9w8PdihBAeR4xgGYWWqBnmDTrpWEW1QjuYAUkR7A6X48q1iQVgN433aSFxQGgtureVz7cCyi5zfuMTtBF3AkanjtvNs9m8u2JobxNfphSi3');
    ...
    const result0 = deriveHdPrivateNodeChild(master.node, hardenedIndex0);
...
test('[crypto] deriveHdPrivateNodeChild: errors', (t) => {
    ...
    t.deepEqual(deriveHdPrivateNodeChild(node, max + 1), HdNodeDerivationError.childIndexExceedsMaximum);
});
test('[crypto] deriveHdPublicNodeChild', (t) => {
    ...
    t.deepEqual(deriveHdPublicNodeChild(parentPublic, derivationIndex), expectedPublic);
    t.deepEqual(deriveHdPublicNodeChild(parentPublic, derivationIndex, crypto), expectedPublic);
});
test('[crypto] deriveHdPublicNodeChild: errors', (t) => {
    const { node } = decodeHdPublicKey(xpub);
    const hardened0 = 0x80000000;
    t.deepEqual(deriveHdPublicNodeChild(node, hardened0), HdNodeDerivationError.hardenedDerivationRequiresPrivateNode);
});
test('[crypto] deriveHdPath', (t) => {
    const { node: privateNode } = decodeHdPrivateKey(xprv);
    const publicNode = deriveHdPublicNode(privateNode);
    t.deepEqual(deriveHdPath(privateNode, 'm'), privateNode);
    t.deepEqual(deriveHdPath(publicNode, 'M', crypto), publicNode);
    t.deepEqual(deriveHdPath(privateNode, "m/0'/1"), {
        ...decodeHdPrivateKey('xprv9w8PdihBAeR4xgGYWWqBnmDTrpWEW1QjuYAUkR7A6X48q1iQVgN433aSFxQGgtureVz7cCyi5zfuMTtBF3AkanjtvNs9m8u2JobxNfphSi3').node,
        parentIdentifier: hexToBin('2f2bc501c943dd7f17904b612c090dd88270cc59'),
    });
    t.deepEqual(deriveHdPath(publicNode, 'M/0/1/2/3'), deriveHdPublicNode(deriveHdPath(privateNode, 'm/0/1/2/3')));
    t.deepEqual(deriveHdPath(privateNode, "m/0'/1'/2'/3'"), deriveHdPath(privateNode, 'm/2147483648/2147483649/2147483650/2147483651'));
});
test('[crypto] deriveHdPath: errors', (t) => {
    const { node: privateNode } = decodeHdPrivateKey(xprv);
    const publicNode = deriveHdPublicNode(privateNode);
    t.deepEqual(deriveHdPath(privateNode, 'm/bad/1'), HdNodeDerivationError.invalidDerivationPath);
    t.deepEqual(deriveHdPath(privateNode, 'M'), HdNodeDerivationError.invalidPrivateDerivationPrefix);
    t.deepEqual(deriveHdPath(publicNode, 'm'), HdNodeDerivationError.invalidPublicDerivationPrefix);
    t.deepEqual(deriveHdPath(privateNode, 'm/0/4294967296/0'), HdNodeDerivationError.childIndexExceedsMaximum);
...
test('[crypto] crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode', (t) => {
    const { node: parentPrivateNode } = decodeHdPrivateKey(xprv);
    ...
    const nonHardenedGrandchildNode = deriveHdPath(hardenedChildNode, 'm/1234');
    t.deepEqual(crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode(parentPublicNode, nonHardenedChildNode), parentPrivateNode);
    t.deepEqual(crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode(hardenedChildPublicNode, nonHardenedGrandchildNode, crypto), hardenedChildNode);
    t.deepEqual(crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode(parentPublicNode, hardenedChildNode), HdNodeCrackingError.cannotCrackHardenedDerivation);
});
const bip32Vector = test.macro({
    ...
    exec: (t, seedHex, path, hdPrivateKey, hdPublicKey) => {
        ...
        t.deepEqual(childNode.parentIdentifier?.slice(0, fingerprintLength), path === 'm' ? undefined : decodedPrivate.node.parentFingerprint);
        ...
        t.deepEqual(publicNode.parentIdentifier?.slice(0, fingerprintLength), path === 'm' ? undefined : decodedPublic.node.parentFingerprint);
...
test('#1.1', bip32Vector, '000102030405060708090a0b0c0d0e0f', 'm', 'xprv9s21ZrQH143K3QTDL4LXw2F7HEK3wJUD2nW2nRk4stbPy6cq3jPPqjiChkVvvNKmPGJxWUtg6LnF5kejMRNNU3TGtRBeJgk33yuGBxrMPHi', 'xpub661MyMwAqRbcFtXgS5sYJABqqG9YLmC4Q1Rdap9gSE8NqtwybGhePY2gZ29ESFjqJoCu1Rupje8YtGqsefD265TMg7usUDFdp6W1EGMcet8');
test('#1.2', bip32Vector, '000102030405060708090a0b0c0d0e0f', "m/0'", 'xprv9uHRZZhk6KAJC1avXpDAp4MDc3sQKNxDiPvvkX8Br5ngLNv1TxvUxt4cV1rGL5hj6KCesnDYUhd7oWgT11eZG7XnxHrnYeSvkzY7d2bhkJ7', 'xpub68Gmy5EdvgibQVfPdqkBBCHxA5htiqg55crXYuXoQRKfDBFA1WEjWgP6LHhwBZeNK1VTsfTFUHCdrfp1bgwQ9xv5ski8PX9rL2dZXvgGDnw');
test('#1.3', bip32Vector, '000102030405060708090a0b0c0d0e0f', "m/0'/1", 'xprv9wTYmMFdV23N2TdNG573QoEsfRrWKQgWeibmLntzniatZvR9BmLnvSxqu53Kw1UmYPxLgboyZQaXwTCg8MSY3H2EU4pWcQDnRnrVA1xe8fs', 'xpub6ASuArnXKPbfEwhqN6e3mwBcDTgzisQN1wXN9BJcM47sSikHjJf3UFHKkNAWbWMiGj7Wf5uMash7SyYq527Hqck2AxYysAA7xmALppuCkwQ');
test('#1.4', bip32Vector, '000102030405060708090a0b0c0d0e0f', "m/0'/1/2'", 'xprv9z4pot5VBttmtdRTWfWQmoH1taj2axGVzFqSb8C9xaxKymcFzXBDptWmT7FwuEzG3ryjH4ktypQSAewRiNMjANTtpgP4mLTj34bhnZX7UiM', 'xpub6D4BDPcP2GT577Vvch3R8wDkScZWzQzMMUm3PWbmWvVJrZwQY4VUNgqFJPMM3No2dFDFGTsxxpG5uJh7n7epu4trkrX7x7DogT5Uv6fcLW5');
test('#1.5', bip32Vector, '000102030405060708090a0b0c0d0e0f', "m/0'/1/2'/2", 'xprvA2JDeKCSNNZky6uBCviVfJSKyQ1mDYahRjijr5idH2WwLsEd4Hsb2Tyh8RfQMuPh7f7RtyzTtdrbdqqsunu5Mm3wDvUAKRHSC34sJ7in334', 'xpub6FHa3pjLCk84BayeJxFW2SP4XRrFd1JYnxeLeU8EqN3vDfZmbqBqaGJAyiLjTAwm6ZLRQUMv1ZACTj37sR62cfN7fe5JnJ7dh8zL4fiyLHV');
test('#1.6', bip32Vector, '000102030405060708090a0b0c0d0e0f', "m/0'/1/2'/2/1000000000", 'xprvA41z7zogVVwxVSgdKUHDy1SKmdb533PjDz7J6N6mV6uS3ze1ai8FHa8kmHScGpWmj4WggLyQjgPie1rFSruoUihUZREPSL39UNdE3BBDu76', 'xpub6H1LXWLaKsWFhvm6RVpEL9P4KfRZSW7abD2ttkWP3SSQvnyA8FSVqNTEcYFgJS2UaFcxupHiYkro49S8yGasTvXEYBVPamhGW6cFJodrTHy');
...
test('#2.3', bip32Vector, 'fffcf9f6f3f0edeae7e4e1dedbd8d5d2cfccc9c6c3c0bdbab7b4b1aeaba8a5a29f9c999693908d8a8784817e7b7875726f6c696663605d5a5754514e4b484542', "m/0/2147483647'", 'xprv9wSp6B7kry3Vj9m1zSnLvN3xH8RdsPP1Mh7fAaR7aRLcQMKTR2vidYEeEg2mUCTAwCd6vnxVrcjfy2kRgVsFawNzmjuHc2YmYRmagcEPdU9', 'xpub6ASAVgeehLbnwdqV6UKMHVzgqAG8Gr6riv3Fxxpj8ksbH9ebxaEyBLZ85ySDhKiLDBrQSARLq1uNRts8RuJiHjaDMBU4Zn9h8LZNnBC5y4a');
test('#2.4', bip32Vector, 'fffcf9f6f3f0edeae7e4e1dedbd8d5d2cfccc9c6c3c0bdbab7b4b1aeaba8a5a29f9c999693908d8a8784817e7b7875726f6c696663605d5a5754514e4b484542', "m/0/2147483647'/1", 'xprv9zFnWC6h2cLgpmSA46vutJzBcfJ8yaJGg8cX1e5StJh45BBciYTRXSd25UEPVuesF9yog62tGAQtHjXajPPdbRCHuWS6T8XA2ECKADdw4Ef', 'xpub6DF8uhdarytz3FWdA8TvFSvvAh8dP3283MY7p2V4SeE2wyWmG5mg5EwVvmdMVCQcoNJxGoWaU9DCWh89LojfZ537wTfunKau47EL2dhHKon');
...
const fcBip32Path = () => fc
    ...
    .join('/')}`);
testProp('[fast-check] [crypto] HD key derivation is equivalent to bitcore-lib-cash', [fcBip32Path()], (t, path) => {
    const privateNode = decodeHdPrivateKey(xprv).node;
    const node = deriveHdPath(privateNode, path);
...
testProp('[fast-check] [crypto] encodeHdPublicKey <-> decodeHdPublicKey', [
    fc.boolean(),
...
], (t, mainnet, depth, childIndex, parentFingerprint, chainCode, publicKey) => {
    ...
    t.deepEqual(encoded, encodeHdPublicKey(decodeHdPublicKey(encoded)));
});
testProp('[fast-check] [crypto] encodeHdPrivateKey <-> decodeHdPrivateKey', [
    fc.boolean(),
    fc.integer({ max: maximumDepth, min: 0 }),
    fc.integer({ max: maximumChildIndex, min: 0 }),
    fc.uint8Array({
    ...
    fc.uint8Array({ maxLength: chainCodeLength, minLength: chainCodeLength }),
    fc.uint8Array({ maxLength: publicKeyLength, minLength: publicKeyLength }),
], (t, mainnet, depth, childIndex, parentFingerprint, chainCode, privateKey) => {
    ...
    t.deepEqual(encoded, encodeHdPrivateKey(decodeHdPrivateKey(encoded)));
});
testProp('[fast-check] [crypto] derive non-hardened HD node <-> crack HD node', [
    fc.integer({ max: maximumDepth, min: 0 }),
...
], (t, depth, childIndexes, parentFingerprint, chainCode, privateKey) => {
    if (!validateSecp256k1PrivateKey(privateKey)) {
        t.pass();
        return;
    }
    ...
    const { node: parentPrivateNode } = decodeHdPrivateKey(parentXprv);
    const parentPublicNode = deriveHdPublicNode(parentPrivateNode);
    const nonHardenedChildNode = deriveHdPrivateNodeChild(parentPrivateNode, childIndexes);
    const crackedParentNode = crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode(parentPublicNode, nonHardenedChildNode);
