...
import { attemptCashAddressFormatErrorCorrection, CashAddressCorrectionError, CashAddressDecodingError, CashAddressNetworkPrefix, CashAddressType, CashAddressTypeBits, CashAddressVersionByte, CashAddressVersionByteDecodingError, decodeBase58AddressFormat, decodeCashAddress, decodeCashAddressFormat, decodeCashAddressFormatWithoutPrefix, decodeCashAddressNonStandard, decodeCashAddressVersionByte, encodeCashAddress, encodeCashAddressFormat, encodeCashAddressNonStandard, encodeCashAddressVersionByte, hexToBin, maskCashAddressPrefix, splitEvery, } from '../lib.js';
...
const lowercaseLetter = () => fc.integer({ max: 122, min: 97 }).map((i) => String.fromCharCode(i));
...
test('encodeCashAddressVersionByte', (t) => {
    t.deepEqual(encodeCashAddressVersionByte(CashAddressTypeBits.p2pkh, 20), CashAddressVersionByte.p2pkh);
    t.deepEqual(encodeCashAddressVersionByte(0, 20), CashAddressVersionByte.p2pkh);
    t.deepEqual(encodeCashAddressVersionByte(CashAddressTypeBits.p2sh, 20), CashAddressVersionByte.p2sh20);
    t.deepEqual(encodeCashAddressVersionByte(1, 20), CashAddressVersionByte.p2sh20);
    t.deepEqual(encodeCashAddressVersionByte(CashAddressTypeBits.p2sh, 32), CashAddressVersionByte.p2sh32);
    t.deepEqual(encodeCashAddressVersionByte(CashAddressTypeBits.p2pkhWithTokens, 20), CashAddressVersionByte.p2pkhWithTokens);
    t.deepEqual(encodeCashAddressVersionByte(CashAddressTypeBits.p2shWithTokens, 20), CashAddressVersionByte.p2sh20WithTokens);
    t.deepEqual(encodeCashAddressVersionByte(CashAddressTypeBits.p2shWithTokens, 32), CashAddressVersionByte.p2sh32WithTokens);
});
test('decodeCashAddressVersionByte', (t) => {
    ...
    t.deepEqual(decodeCashAddressVersionByte(CashAddressVersionByte.p2pkhWithTokens), { length: 20, typeBits: 2 });
    t.deepEqual(decodeCashAddressVersionByte(CashAddressVersionByte.p2sh20WithTokens), { length: 20, typeBits: 3 });
    t.deepEqual(decodeCashAddressVersionByte(CashAddressVersionByte.p2sh32WithTokens), { length: 32, typeBits: 3 });
...
test('encodeCashAddress: works', (t) => {
    ...
    t.deepEqual(encodeCashAddress('bitcoincash', CashAddressType.p2pkh, payload), 'bitcoincash:qq2azmyyv6dtgczexyalqar70q036yund54qgw0wg6');
    t.deepEqual(encodeCashAddress('bitcoincash', 'p2pkh', payload), 'bitcoincash:qq2azmyyv6dtgczexyalqar70q036yund54qgw0wg6');
    t.deepEqual(encodeCashAddress('bitcoincash', CashAddressType.p2pkhWithTokens, payload), 'bitcoincash:zq2azmyyv6dtgczexyalqar70q036yund5j2mspghf');
    t.deepEqual(encodeCashAddress('bitcoincash', 'p2pkhWithTokens', payload), 'bitcoincash:zq2azmyyv6dtgczexyalqar70q036yund5j2mspghf');
    t.deepEqual(encodeCashAddress('bchreg', 'p2pkh', payload), 'bchreg:qq2azmyyv6dtgczexyalqar70q036yund5tw6gw2vq');
    t.deepEqual(encodeCashAddress('bchreg', 'p2pkh', payload), 'bchreg:qq2azmyyv6dtgczexyalqar70q036yund5tw6gw2vq');
    t.deepEqual(encodeCashAddressFormat('bitauth', encodeCashAddressVersionByte(0, 32), hexToBin('978306aa4e02fd06e251b38d2e961f78f4af2ea6524a3e4531126776276a6af1')), 'bitauth:qwtcxp42fcp06phz2xec6t5krau0ftew5efy50j9xyfxwa38df40zp58z6t5w');
    t.throws(() => encodeCashAddress('bitcoincash', 'p2sh', hexToBin('97')), {
        message: 'Error encoding CashAddress: a payload of this length can not be encoded as a valid CashAddress. Payload length: 1.',
    });
});
test('decodeCashAddress: works', (t) => {
    ...
    t.deepEqual(decodeCashAddress('bitcoincash:qq2azmyyv6dtgczexyalqar70q036yund54qgw0wg6'), { payload, prefix: 'bitcoincash', type: CashAddressType.p2pkh });
    t.deepEqual(decodeCashAddress('bchreg:qq2azmyyv6dtgczexyalqar70q036yund5tw6gw2vq'), {
        payload,
        prefix: CashAddressNetworkPrefix.regtest,
        type: CashAddressType.p2pkh,
    });
    t.deepEqual(decodeCashAddress('bchreg:qq2azmyyv6dtgczexyalqar70q036yund5tw6gw2vq'), { payload, prefix: 'bchreg', type: CashAddressType.p2pkh });
    t.deepEqual(decodeCashAddressFormat('bitauth:qwtcxp42fcp06phz2xec6t5krau0ftew5efy50j9xyfxwa38df40zp58z6t5w'), {
        payload: hexToBin('978306aa4e02fd06e251b38d2e961f78f4af2ea6524a3e4531126776276a6af1'),
        prefix: 'bitauth',
    ...
    t.deepEqual(decodeCashAddress('bitcoincash:qu2azmyyv6dtgczexyalqar70q036yund53an46hf6'), CashAddressDecodingError.mismatchedPayloadLength);
});
test('CashAddress test vectors', (t) => {
    Object.values(cashAddrJson)
        ...
        const typeBits = vector.type;
        const version = encodeCashAddressVersionByte(typeBits, payload.length);
...
test('decodeCashAddressWithoutPrefix', (t) => {
    const payload = hexToBin('15d16c84669ab46059313bf0747e781f1d13936d');
    t.deepEqual(decodeCashAddressFormatWithoutPrefix('qq2azmyyv6dtgczexyalqar70q036yund53jvfde0x'), { payload, prefix: 'bchtest', version: 0 });
    t.deepEqual(decodeCashAddressFormatWithoutPrefix('qq2azmyyv6dtgczexyalqar70q036yund54qgw0wg6'), { payload, prefix: 'bitcoincash', version: 0 });
    t.deepEqual(decodeCashAddressFormatWithoutPrefix('qq2azmyyv6dtgczexyalqar70q036yund5tw6gw2vq'), { payload, prefix: 'bchreg', version: 0 });
...
test('[fast-check] encodeCashAddressFormat <-> decodeCashAddressFormat', (t) => {
    const roundTripWithPayloadLength = (length) => fc.property(fc
        .array(lowercaseLetter(), { maxLength: 50, minLength: 1 })
        .map((arr) => arr.join('')), fc.nat(0xff), fc.uint8Array({ maxLength: length, minLength: length }), (prefix, version, payload) => {
        t.deepEqual(decodeCashAddressFormat(encodeCashAddressFormat(prefix, version, payload)), { payload, prefix, version });
    });
    t.notThrows(() => {
...
test('[fast-check] encodeCashAddressNonStandard <-> decodeCashAddressNonStandard', (t) => {
    const roundTripWithPayloadLength = (length) => fc.property(fc
        .array(lowercaseLetter(), { maxLength: 50, minLength: 1 })
        .map((arr) => arr.join('')), fc.nat(15), fc.uint8Array({ maxLength: length, minLength: length }), (prefix, typeBits, payload) => {
        t.deepEqual(decodeCashAddressNonStandard(encodeCashAddressNonStandard(prefix, typeBits, payload)), { payload, prefix, typeBits });
    });
    t.notThrows(() => {
        fc.assert(roundTripWithPayloadLength(20));
        fc.assert(roundTripWithPayloadLength(24));
...
test('[fast-check] encodeCashAddress <-> decodeCashAddress', (t) => {
    const prefixes = Object.keys(CashAddressNetworkPrefix);
    const types = Object.keys(CashAddressType);
    const roundTripWithPayloadLength = (length) => fc.property(fc.nat(prefixes.length - 1), fc.nat(types.length - 1), fc.uint8Array({ maxLength: length, minLength: length }), (prefixIndex, typeIndex, payload) => {
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        const prefix = prefixes[prefixIndex];
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        const type = types[typeIndex];
        t.deepEqual(decodeCashAddress(encodeCashAddress(prefix, type, payload)), { payload, prefix, type });
    });
    t.notThrows(() => {
        fc.assert(roundTripWithPayloadLength(20));
        fc.assert(roundTripWithPayloadLength(24));
...
test('[fast-check] attemptCashAddressErrorCorrection', (t) => {
    const correctsUpToTwoErrors = (payloadLength) => fc.property(fc
        .array(lowercaseLetter(), { maxLength: 50, minLength: 1 })
        .map((arr) => arr.join('')), fc.nat(15), fc.uint8Array({ maxLength: payloadLength, minLength: payloadLength }), fc.array(fc.nat(payloadLength), { maxLength: 2, minLength: 0 }), 
    // eslint-disable-next-line @typescript-eslint/max-params
    (prefix, typeBits, payload, randomErrors) => {
        const address = encodeCashAddressNonStandard(prefix, typeBits, payload);
...
const legacyVectors = test.macro({
    ...
    title: (_, base58Address) => `CashAddress <-> Legacy Base58 Vectors: ${base58Address}`,
});
test(legacyVectors, 
// cspell: disable-next-line
...
test(legacyVectors, 
// cspell: disable-next-line
'1Q2TWHE3GMdB6BZKafqwxXtWAWgFt5Jvm3', 'bitcoincash:qr7fzmep8g7h7ymfxy74lgc0v950j3r2959lhtxxsl');
