...
import { createVirtualMachineBCH2022, createVirtualMachineBCH2023, hexToBin, readTransactionCommon, readTransactionNonTokenAware, readTransactionOutputs, readTransactionOutputsNonTokenAware, stringify, stringifyDebugTraceSummary, summarizeDebugTrace, } from '../lib.js';
...
const testVm = ({ fails, succeeds, vm, vmName, vmOptions, }) => {
    const runCase = test.macro({
        // eslint-disable-next-line complexity
        exec: (t, testCase, expectedToSucceed) => {
            const [shortId, description, unlockingAsm, lockingAsm, txHex, sourceOutputsHex, inputIndex,] = testCase;
            const testedIndex = inputIndex ?? 0;
            const supportsTokens = vmOptions?.canParseTokens !== false;
            const sourceOutputsRead = (supportsTokens
                ? readTransactionOutputs
                : readTransactionOutputsNonTokenAware)({ bin: hexToBin(sourceOutputsHex), index: 0 });
            const transactionRead = (supportsTokens ? readTransactionCommon : readTransactionNonTokenAware)({ bin: hexToBin(txHex), index: 0 });
            ...
            const logDebugInfo = () => {
                ...
                t.deepEqual(evaluateResult, debugResult[debugResult.length - 1], `vm.evaluate and the final result of vm.debug differ: is something being unexpectedly mutated? evaluateResult:\n\n${stringify(evaluateResult)}\n\nFinal debugResult:\n\n${stringify(debugResult[debugResult.length - 1])}`);
            ...
            if (expectedToSucceed && typeof result === 'string') {
                ...
                if (failingIndex !== undefined &&
                    ...
                    t.fail(`An unexpected input index caused VMB test "${shortId}" to fail for ${vmName}: the input index under test is ${testedIndex}, but input index ${failingIndex} failed. Error: ${result}`);
                    t.log(`Failing input at index ${failingIndex}:`, stringifyDebugTraceSummary(summarizeDebugTrace(vm.debug({
                ...
                logDebugInfo();
                t.fail(`VMB test "${shortId}" - "${description}" - for ${vmName} is expected to succeed but failed. Error: ${result}`);
                return;
            }
            if (!expectedToSucceed && typeof result !== 'string') {
                logDebugInfo();
                t.fail(`VMB test "${shortId}" - "${description}" - for ${vmName} is expected to fail but succeeded.`);
                return;
            }
            t.pass();
        },
        ...
        caseNumberOfCaseCount = '(unknown/unknown)', [shortId, description]) => `[vmb_tests] [${vmName}] ${shortId} ${caseNumberOfCaseCount}: ${description}`,
    });
    ...
    expectedPass.forEach((testCase, index) => {
        (debug === undefined
            ...
            : vmName === debug.vmName && debug.testId === testCase[0]
                ? test
                : test.skip)(`(${index + 1}/${expectedPass.length + expectedFail.length} valid)`, runCase, testCase, true);
    });
    expectedFail.forEach((testCase, index) => {
        (debug === undefined
            ...
            : vmName === debug.vmName && debug.testId === testCase[0]
                ? test
                : test.skip)(`(${expectedPass.length + index + 1}/${expectedPass.length + expectedFail.length} invalid)`, runCase, testCase, false);
    });
};
testVm({
    fails: [
        vmbTestsBCH2022InvalidJson,
        vmbTestsBCH2022NonstandardJson,
        vmbTestsBCHBeforeChipCashtokensInvalidJson,
        vmbTestsBCHBeforeChipCashtokensNonstandardJson,
    ],
    succeeds: [
        vmbTestsBCH2022StandardJson,
        vmbTestsBCHBeforeChipCashtokensStandardJson,
    ],
...
testVm({
    fails: [
        vmbTestsBCHChipCashtokensInvalidJson,
        vmbTestsBCHChipCashtokensNonstandardJson,
    ],
    succeeds: [vmbTestsBCHChipCashtokensStandardJson],
    vm: createVirtualMachineBCH2023(true),
    vmName: 'bch_2023_standard',
});
testVm({
    fails: [vmbTestsBCHChipCashtokensInvalidJson],
    succeeds: [
        vmbTestsBCHChipCashtokensStandardJson,
        vmbTestsBCHChipCashtokensNonstandardJson,
    ],
...
testVm({
    fails: [
        vmbTestsBCHChipLoopsInvalidJson,
        vmbTestsBCHChipLoopsNonstandardJson,
    ],
    succeeds: [vmbTestsBCHChipLoopsStandardJson],
    vm: createVirtualMachineBCHCHIPs(true),
    vmName: 'bch_chips_standard',
});
testVm({
    fails: [vmbTestsBCHChipLoopsInvalidJson],
    succeeds: [
        vmbTestsBCHChipLoopsStandardJson,
        vmbTestsBCHChipLoopsNonstandardJson,
    ],
    vm: createVirtualMachineBCHCHIPs(false),
    vmName: 'bch_chips_nonstandard',
});
//# sourceMappingURL=bch-vmb-tests.spec.js.map