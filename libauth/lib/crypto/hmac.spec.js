...
import { binToHex, hexToBin, hmacSha256, hmacSha512, sha256, sha512, } from '../lib.js';
import { fc, testProp } from '@fast-check/ava';
const vectors = test.macro({
    exec: (t, vector) => {
        t.deepEqual(hmacSha256(hexToBin(vector.secret), hexToBin(vector.message), sha256), hexToBin(vector.sha256));
        t.deepEqual(hmacSha512(hexToBin(vector.secret), hexToBin(vector.message), sha512), hexToBin(vector.sha512));
    },
...
test('6', vectors, {
    ...
    secret: '',
...
testProp('[fast-check] [crypto] hmacSha256 is equivalent to Node.js native HMAC-SHA256', [
    fc.uint8Array({ maxLength: 100, minLength: 1 }),
    fc.uint8Array({ maxLength: 100, minLength: 1 }),
], (t, secret, message) => {
    t.deepEqual(binToHex(hmacSha256(secret, message)), createHmac('sha256', secret).update(message).digest('hex'));
});
testProp('[fast-check] [crypto] hmacSha512 is equivalent to Node.js native HMAC-SHA512', [
    fc.uint8Array({ maxLength: 100, minLength: 0 }),
    fc.uint8Array({ maxLength: 100, minLength: 0 }),
], (t, secret, message) => {
    t.deepEqual(binToHex(hmacSha512(secret, message)), createHmac('sha512', secret).update(message).digest('hex'));
});
